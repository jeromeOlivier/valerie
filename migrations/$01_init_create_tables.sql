# BOOKS
# information needed to display a book on the website.
...
(
    ...
    title         VARCHAR(255) NOT NULL UNIQUE COMMENT 'title of the book',
    background    VARCHAR(255) NOT NULL COMMENT 'css gradient background color for web page',
...
# availability scope is used to display the geographic availability of the book
...
# languages are used to display the language of the book
...
# book formats describe the two formats a book can be purchased in (pdf or physical)
...
(
    ...
    in_stock BOOLEAN        NOT NULL DEFAULT TRUE COMMENT 'whether the book is in stock',
...
(
    id          INT AUTO_INCREMENT PRIMARY KEY,
    book_id     INT          NOT NULL COMMENT 'book id',
    seq_order   INT          NOT NULL COMMENT 'order of the workbook in the book',
...
(
    ...
    book_format_id INT            NOT NULL COMMENT 'book format id',
    quantity       INT            NOT NULL CHECK (quantity > 0) COMMENT 'number of books ordered',
    unit_price     DECIMAL(10, 2) NOT NULL COMMENT 'price of book at time of purchase',
    FOREIGN KEY (order_id) REFERENCES orders (id),
    FOREIGN KEY (book_format_id) REFERENCES book_formats (id),
    UNIQUE KEY (order_id, book_format_id) COMMENT 'order id and book format id must be unique'
);
